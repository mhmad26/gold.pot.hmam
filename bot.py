...
def get_gold_price():
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as response:
            return float(json.loads(response.read().decode())['chart']['result'][0]['meta']['regularMarketPrice'])
    except: return 0.0
        ...
        with urllib.request.urlopen(url, timeout=5) as res:
            ...
            for update in data.get("result", []):
                ...
                if "callback_query" in update:
                    ...
                    if val == "price": send_menu(f"السعر الحالي: {price}")
                    elif val == "analysis": send_menu(f"السوق مستقر عند: {price}")
    except: pass
    time.sleep(2)
